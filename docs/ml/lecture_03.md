...
- **Классификация (Classification):**
  - $Y$ — конечное множество категорий (классов). ${\text{"спам"}, \text{"не спам"}}$ или $Y = {0, 1, \dots, K}$, где $0, \dots , K$ – номера классов.
  - *Задача*: отнести объект к одному из классов.
- **Регрессия (Regression):**
  - $Y$ — непрерывное числовое множество, $\mathbb{R}$.
...
- **Примеры регрессии:**
  ...
  - Предсказание спроса на товар в зависимости от сезона и активных рекламных кампаний;
...
1. **Вырожденность матрицы:** Если матрица $X^T X$ является вырожденной (определитель равен нулю), то обратной матрицы не существует. Это происходит когда:
   ...
   - Количество признаков превышает количество объектов ($d > N$)
...
$$
R2(y,\hat{y})=1−\frac{\sum^n_{i=1} (y_i - \hat{y}_i)}{\sum^n_{i=1} (y_i - \bar{y}_i)}
$$
...
**Градиент логистической функции потерь** вычисляется так:
...
- не учета цены ошибки на объектах разных классов: ложноположительное или ложноотрицательное предсказание в конкретной задаче могут иметь разные
...
В каждой задаче считается своя матрица ошибок. Затем есть два варианта получения итогового значения метрики из $K$ матриц ошибок:
