...
В зависимости от природы имеющихся данных и задачи, которую мы должны решить, машинное обучение классифицируются на следующие виды (которые иногда мы будем называть парадигмами):

- обучение с учителем;
- обучение без учителя;
- обучение с подкреплением.

Рассмотрим типичные задачи, решаемые в рамках этих парадигм.
...
В обучении с подкреплением мы располагаем агентом, который обучается, взаимодействуя со средой. На каждом шаге агент оказывает в некотором состоянии $s_t$, совершает действие $a_t$, получает от среды награду (reward) $r_t$ и переходит в новое состояние $s_{t+1}$. Цель агента – выработать стратегию (policy) $\pi$, максимизирующую суммарную награду (cumulative reward) за всё время.

## Как решать задачу с помощью машинного обучения?

Решение (практически) любой задачи машинного обучения можно систематизировать, опираясь на шесть ключевых характеристик: три из которых описывают саму проблему, а три — стратегию её решения.

**Характеристики проблемы:**

1. **Класс задачи (Problem class):** Первый шаг — это точная постановка задачи. Необходимо определить, какой тип машинного обучения ей соответствует (с учителем, без учителя, с подкреплением или другой), какого вида данные доступны для обучения и какие запросы будут поступать к системе на этапе эксплуатации.
...
3. **Критерии оценки (Evaluation criteria):** Необходимо определить, как будет измеряться правильность решения задачи. Этот этап включает выбор как метрик для оценки качества предсказаний модели машинного, таки и общих, бизнес метрик.
...
- Выбросы (аномалии) – объекты, которые резко отличаются от большинства остальных по статистическим характеристикам, например, зарплата человека в $5 млрд., когда средняя ЗП в компании – тысяча рублей.
